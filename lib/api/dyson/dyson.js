...
async function _sensors(req, res, next) {
  ...
  const { durationspan, device } = req.params;
    ...
    switch (durationspan) {
      case 'year':
        ...
        aggregate = [
          ...
          { $match: { time: { $gt: timeBucket }, lower_name: device } },
          {
            $group: {
              _id: '$Year',
              device: { $last: '$device' },
      ...
      case 'week':
        ...
        break;
      case '2days':
        timeBucket = moment().utc().subtract(2, 'day').toDate();
        aggregate = [
          {
            $addFields: {
              Hour: { $hour: '$time' },
...
async function _current(req, res, next) {
  this.logger.debug(
    `${this._traceStack()} - Display Dyson latest readings API called`,
  );
  let dbConnection;
