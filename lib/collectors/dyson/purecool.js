...
async function processPureCoolData(room) {
  ...
  const dysonIP = await this._getVaultSecret.call(
    this,
    process.env.ENVIRONMENT,
    `Dyson${room}IP`,
  );
  ...
  this.logger.debug(`${this._traceStack()} - Connecting to Dyson device`);
  ...
  mqttClient.on('message', async (topic, message) => {
    ...
    if (deviceData.msg === 'ENVIRONMENTAL-CURRENT-SENSOR-DATA') {
      ...
      /*
        ...
        7-9 = High
      */
      const airQuality = Math.max(pm25, pm10, va10, noxl, pact, vact);
...
async function _getPureCoolData() {
  ...
  await processPureCoolData.call(this, 'Office');
  await processPureCoolData.call(this, 'Bedroom');
