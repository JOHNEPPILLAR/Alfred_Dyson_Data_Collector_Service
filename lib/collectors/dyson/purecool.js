...
async function _getDysonDeviceData() {
  ...
  try {
    ...
    const baseURL = `https://appapi.cp.dyson.com/v1/userregistration`;

    const checkUserURL = `${baseURL}/userstatus?country=GB&email=${dysonUserName}`;
    const checkUser = await this._callAPIServiceGet.call(this, checkUserURL);
    ...
    debug(`Login to Dyson cloud`);
    const loginURL = `${baseURL}/authenticate?country=GB`;
    const body = {
      Email: dysonUserName,
      Password: dysonPassword,
    };
    const login = await this._callAPIServicePost.call(this, loginURL, body);
